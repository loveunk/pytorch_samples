...
# Init wandb
import wandb
wandb.init(project="pytorch_mnist")
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        wandb.log({"Loss_train": loss.item()}, step=batch_idx + 1 + (epoch - 1) * int(math.ceil(len(train_loader.dataset) / args.batch_size)))
...
def main():
    ...
    if args.save_model:
        ...
        torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'mnist_cnn.pt'))
